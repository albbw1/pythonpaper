    ...
    def __init__(self, config):
        ...
        if self.backbone_arch in dir(models):
            ...
            if self.backbone_arch in pretrained_model:
                self.model.load_state_dict(torch.load(pretrained_model[self.backbone_arch]),False)
                for name, param in self.model.named_parameters():
                    param.requires_grad = False
        else:
            if self.backbone_arch in pretrained_model:
                #self.model = pretrainedmodels.__dict__[self.backbone_arch](num_classes=220, pretrained=None)
                self.model = pretrainedmodels.__dict__[self.backbone_arch](num_classes=1000, pretrained=None)
            else:
                #self.model = pretrainedmodels.__dict__[self.backbone_arch](num_classes=220)
